...
- **b5 / b13:** 上記のどのケースでも（`sus4`/`sus2`/パワーコードを含む）、独立した装飾として末尾に追加され、黙って無視されることはない。`b5`はフラット5度、`b13`は「5度が同時に含まれている場合」の6度の音（5度が無い場合、同じ音は`#5`＝オーギュメントとして解釈される。[度数の自動計算とルート強調](#度数の自動計算とルート強調)参照）。例: `C9(b5)`, `Cm7(9, b5)`, `Csus4(b13)`（jazzスタイルでは`C9b5`, `Csus4b13`）。
