...
*   指定できるキーはSystem設定と全く同じ（2.1節の全項目: orientation, strings, fretCount, stringSpacing, fretSpacing, labelMode, accidental, defaultShape, fillStyle, nutStyle, fretNumbering, defaultTuning, omittedStringBehavior, noteSize, labelFontSize, namingMode, chordSymbolStyle）。
...
1.  **分数コード (Slash Chords) の判定と表記:**
    実際に描画される最も低いピッチの音（最も太い弦の押弦位置/開放弦）が Root 音でない場合、推論されたコード名の末尾に `/` とベース音を付与すること。
...
3.  **テンションと Add の区別 (7thの有無と省略ルール):**
    ...
    *   **b5（オルタード）の扱い:** 上記いずれのケースでも、b5が同時に含まれる場合は独立した装飾として末尾に追加する。`Chord Symbol Style`が`standard`/`berklee`ならカッコ付き（例: `C9(b5)`, `Cm7(9, b5)`）、`jazz`ならカッコなしでそのまま連結する（例: `C9b5`）。
4.  **6th と 6/9 コード:**
    *   7度が無く、6度(M6)が含まれる場合は `6`（例: `C6`, `Cm6`）とする。
    *   上記に加えて9度(M2)も含まれる場合は `6/9` とする。
    *   このケースは3.のテンション/カッコの対象外（常にカッコなし）。
5.  **表記の流儀 (Chord Symbol Style):**
    ...
    *   `jazz`: `ø7`, `°7`, `Δ7`, `-`。3.で追加されるテンション/b5にカッコは付けず、そのまま連結する。
