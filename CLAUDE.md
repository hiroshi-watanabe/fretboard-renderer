...
3.  **テンションと Add の区別 (7thの有無と省略ルール):**
    ...
    *   **b5 / b13（オルタード）の扱い:** 3.のテンション付きコードに限らず、2.のsus/パワーコードを含むすべてのケースで、b5、および5度(P5)が同時に含まれる場合に限りm6を♭13として扱ったものが構成音に含まれる場合、それらを独立した装飾として末尾に追加する（黙って無視しないこと）。`Chord Symbol Style`が`standard`/`berklee`ならカッコ付き（例: `C9(b5)`, `Cm7(9, b5)`, `Csus4(b13)`）、`jazz`ならカッコなしでそのまま連結する（例: `C9b5`, `Csus4b13`）。
        *   5度(P5)が含まれない状態でのm6は増5度（augmented）の一部として解釈されるため、この♭13ルールの対象外（4.2節の度数表参照。1オクターブ内の度数体系ではm6とオルタードな♭13/#5は同じ度数ラベルで表現されるため、5度の有無で意味を判定する）。
4.  **6th と 6/9 コード:**
