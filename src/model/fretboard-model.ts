...
import { findBestFitScale, rankBestFitScales } from "../music/scales";
...
export interface ResolvedNote {
	...
	className?: string;
	isRoot: boolean;
	/** True for a reference pitch not physically fretted/sounding — rendered as its label
	 * in parentheses with no shape, and excluded from the bass/slash-chord calculation. */
	virtual: boolean;
...
export interface ResolvedModel {
	...
	title: string;
	/**
	 * Same content as `title`, but — only when `titleIsChordTypography` is true — with
	 ...
	 * text, not necessarily a chord name) or to a scale-mode title (a plain sentence like
	 * "E Minor Pentatonic (+2)" that happens to contain digits/`+`/`b`, none of which are
	 * chord-typography roles) — only an auto-generated *chord* title gets that treatment.
	 */
	titleMarkup: string;
	titleIsAutoGenerated: boolean;
	/** True only for an auto-generated *chord*-mode title — the one case where `titleMarkup`
	 *  carries real typography markers the renderer should interpret (see `titleMarkup`).
	 *  Scale-mode titles (single best-fit or multi-line ranked) and user-supplied titles are
	 *  always plain text. */
	titleIsChordTypography: boolean;
	stringsTotal: number;
	visibleStart: number;
	visibleEnd: number;
	startFret: number;
...
export function resolveFretboardModel(
	...
	// Whether any *physically sounding* (non-virtual) note actually lands on the root's
	// pitch — a virtual root still fully participates in presentDegrees/interval math (it
	// IS the reference point), but "the root itself isn't really fretted" is a fact only
	// this flag captures, used for the "(omit1)" title marker below.
	let rootPhysicallyPresent = false;
	const noteComputations = visibleNotes.map((n) => {
		...
		if (delta !== undefined) {
			...
			if (delta === 0 && !n.virtual) rootPhysicallyPresent = true;
		}
		return { n, pitchClass, delta };
	});
	const rootOmitted = rootPitchClass !== undefined && !rootPhysicallyPresent;
	...
	// In "scale" mode, notes whose degree falls outside the best-fit scale (see
	// `findBestFitScale`) are the title's "passing notes" — flag them here too so the
	// diagram visually distinguishes them (ghosted, like a manually-marked `ghost: true`
	// note) without the user having to mark each one by hand (CLAUDE.md §4.4).
	const scaleOutliers: ReadonlySet<string> | undefined =
		namingMode === "scale" && rootPitchClass !== undefined
			? new Set(findBestFitScale(presentDegrees).outliers)
			: undefined;
	...
	const resolvedNotes: ResolvedNote[] = noteComputations.map(({ n, pitchClass, delta }) => {
		...
		// A "passing note" under `scale` mode: a physically-sounding note whose degree
		// isn't covered by the best-fit scale. Rendered ghosted like a manually-marked
		// `ghost: true` note (see `ghost` below), plus its own class so it can be told
		// apart from a note the user ghosted for some unrelated reason.
		const isPassingTone = !n.virtual && delta !== undefined && (scaleOutliers?.has(degreeForDelta(delta)) ?? false);
		const ghost = (n.ghost ?? false) || isPassingTone;

		const classNames: string[] = [];
		if (n.class) classNames.push(n.class);
		if (isRoot) classNames.push("fretboard-note--root");
		if (ghost) classNames.push("fretboard-note--ghost");
		if (isPassingTone) classNames.push("fretboard-note--passing");
	...
	if (rootPitchClass !== undefined) {
		// A virtual note is never physically sounding, so it can never be "the lowest
		// sounding note" that defines a slash chord's bass.
		const soundingNotes = visibleNotes.filter(
			(n): n is NoteEntry & { f: number } => typeof n.f === "number" && !n.virtual
		);
		if (soundingNotes.length > 0) {
			...
			if (bassPitchClass !== rootPitchClass) {
				const bassDegree = degreeForDelta(intervalDelta(rootPitchClass, bassPitchClass));
				const showInversions = config.showInversions ?? settings.showInversions;
				// An inversion (bass = the chord's own 3rd/5th/7th) is real-but-often-
				// unnotated information — skip it unless the setting says otherwise. A
				// bass note that ISN'T one of the chord's own tones is a true slash
				// chord and always shown, since the plain symbol can't otherwise convey it.
				if (showInversions || !isInversionBass(bassDegree)) {
					bassName = isRelative
	...
	const omitNotation = config.omitNotation ?? settings.omitNotation;
	...
	const titleMarkup =
		...
		autoTitle(
			...
			omitNotation,
			rootOmitted,
			config.scaleAnalyze ?? false
		);
	const title = stripTitleMarkers(titleMarkup);
	const titleIsChordTypography = titleIsAutoGenerated && namingMode === "chord";
...
/** Top-N shown by `scaleAnalyze`'s ranked list — "5位くらいまで" per the feature request. */
const SCALE_ANALYZE_RANK_COUNT = 5;
...
function autoTitle(
	...
	const rootLabel = isRelative ? "□" : pitchClassToName(rootPitchClass, settings.accidental);

	// "scale" mode always finds a best-fit scale (never falls back to chord naming) — it's
	// meant for reverse-engineering a phrase/solo against its backing-chord root (CLAUDE.md
	// §4.4), so it always commits to its best guess, flagging leftover notes as "passing
	// notes" rather than refusing to name anything. Omit notation is a chord-specific
	// concept, so it's never applied here.
	if (namingMode === "scale") {
		const formatMatch = (name: string, outliers: readonly string[]): string =>
			outliers.length > 0 ? `${rootLabel} ${name} (+${outliers.join(", ")})` : `${rootLabel} ${name}`;

		// `scaleAnalyze` swaps the single best-fit title for a stacked, ranked list (one
		// line per candidate) — see `ResolvedModel.title`'s multi-line handling and
		// `render-fretboard.ts`, which lays out one line per "\n". Only makes sense as an
		// explicit, per-diagram "inspect this" toggle, so it's Local-only (no System/Global
		// default): rendering every scale-mode diagram in a vault as a 5-line block by
		// default would be far too noisy for anything meant to sit in a real note.
		if (scaleAnalyze) {
			return rankBestFitScales(presentDegrees, SCALE_ANALYZE_RANK_COUNT)
				.map((r) => `${r.rank}. ${formatMatch(r.name, r.outliers)}`)
				.join("\n");
		}

		const { name, outliers } = findBestFitScale(presentDegrees);
		return formatMatch(name, outliers);
	}

	const suffix = inferChordSuffix(presentDegrees, chordSymbolStyle, bassName, omitNotation, rootOmitted);
	if (isRelative) {
		return `□${suffix}`;
	}
