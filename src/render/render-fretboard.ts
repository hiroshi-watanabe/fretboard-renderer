...
function buildFretboardSvg(model: ResolvedModel): SVGSVGElement {
	...
	if (model.title) {
		...
		// A user-typed `title:` is arbitrary text, not necessarily a chord name — parsing
		// it for typography (treating a stray "b" or digit as a flat/tension) would
		// misrender ordinary words. Only an auto-generated title carries markers for that.
		if (model.titleIsAutoGenerated) {
 ...
 */
function renderTitleText(textEl: SVGTextElement, markup: string): void {
	...
	for (const r of runs) {
		if (r.tier === "normal") {
			textEl.appendChild(document.createTextNode(r.text));
			continue;
		}
